...
use crate::{admission, naming, paths, resources};
...
pub fn doctor(cfg_path: &Path) -> Result<()> {
    ...
    if let Some(cfg) = cfg {
        ...
        check("base VM", mp.exists(&cfg.base.name), cfg.base.name.clone());
        match cfg
            .token()
...
pub fn install(cfg_path: &Path, opts: &InstallOpts) -> Result<()> {
    ...
    if opts.dry_run {
        ...
        println!("installed and started {}", naming::SUPERVISOR_UNIT);
    }
...
pub fn uninstall(cfg_path: &Path, opts: &UninstallOpts) -> Result<()> {
    ...
    // 1. stop supervisor + workers
    let _ = sd.disable_now(naming::SUPERVISOR_UNIT);
    let _ = std::fs::remove_file(paths::systemd_user_dir().join(naming::SUPERVISOR_UNIT));
    ...
    for (name, _) in mp.list_prefixed(naming::VM_PREFIX)? {
        if let Some(cfg) = &cfg {
            if name == cfg.base.name && !opts.purge_all {
                continue; // keep base unless purge_all
            }
        }
...
pub fn build_base(cfg_path: &Path, force: bool) -> Result<()> {
    ...
    // base VM must be Stopped to be a clone source
    println!("stopping base (clone source must be stopped)");
    let _ = mp.stop(&base.name);
    // sanity: ensure it's not left Running
